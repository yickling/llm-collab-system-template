...
This protocol defines operating terms so multiple developers (human or LLM) can work on the same codebase without corrupting state, duplicating effort, or losing auditability. It assumes developers work **sequentially** (one active session at a time) and hand off via documented snapshots.
...
1. **Soft lock model**: first developer to claim a file owns it for the active task window.
...
2. Commits must be **atomic** and scoped to one concern.
3. Commit messages follow `<type>: <summary>` format.
   - Types: `feat`, `fix`, `refactor`, `test`, `docs`, `chore`, `ci`
   - Example: `feat: add user authentication endpoint`
4. Do not include unrelated workspace changes in a commit.
5. Never amend or rewrite another developer's commit history unless explicitly requested by a human.
...
- [ ] Claimed scope recorded in a new `SESSION_CLAIM.md`
...
- [ ] Handoff note updated (`SESSION_END.md`)
- [ ] Audit trail updated
